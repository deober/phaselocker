...
def _inside_convex_hull(points: np.ndarray, test_points: np.ndarray) -> List[bool]:
...
def lower_hull(
    ...
    lower_hull_simplices : np.ndarray of ints, shape (n_simplices,)
        Indices of lower convex hull vertex indices grouped by simplices (facets) of the lower convex hull.
    """
...
def _simplex_energy_equation_matrix(
    convex_hull: ConvexHull,
...
def _lower_hull_simplex_containing(
    ...
    out_of_bounds = ~np.array(
        _inside_convex_hull(convex_hull.points[convex_hull.vertices, :-1], compositions)
    )
    ...
    lower_hull_equation_matrix = _simplex_energy_equation_matrix(
...
def lower_hull_energies(
    ...
    np.ndarray of floats, shape (n_points,)
        Energies of points.
    """
    return _lower_hull_simplex_containing(
...
def hull_distance_correlations(
    ...
    ----------
    test_corr: np.array
        mxk correlation matrix, where m is the number of configurations and k is the number of ECI.
        A 1xk hull-distance-correlation vector will be calculated for each of the m correlation vectors provided.
    test_comp: np.array
        mxc matrix of compositions, where m is the number of configurations and c is the number of composition axes.
        Each of the m structures in test_corr should have a 1xc composition vector at the same index.
    reference_hull:np.ndarray
        Convex hull against which each of the m test points will be compared to calculate hull-distance-correlations.
        Can be constructed using the same set of m points as the test set, or using a different set of n points.
    reference_corr:np.ndarray
        nxk correlation matrix, where n is the number of configurations, and k is the number of ECI. Should be the same points that
        were used to construct reference_hull.


    Returns
    -------
    hulldist_corr: np.array
        mxk matrix of effective correlations describing hull distance instead of absolute formation energy; m is the number of
        configurations and k is the number of ECI.
    """

    # Get convex hull simplices
    _, lower_simplices = lower_hull(reference_hull)

    hulldist_corr = np.zeros(test_corr.shape)

    for config_index in list(range(test_corr.shape[0])):
        ...
        relevant_simplex_index, _ = _lower_hull_simplex_containing(
            compositions=test_compositions[config_index].reshape(1, -1),
            convex_hull=reference_hull,
        ...
        simplex_corners = reference_hull.points[
            reference_hull.simplices[relevant_simplex_index]
        ][:, 0:-1]
        interior_point = np.array(
            test_compositions[config_index]
        ...
        hulldist_corr[config_index] = (
            test_corr[config_index]
            - weights @ reference_corr[reference_hull.simplices[relevant_simplex_index]]
        )
    return hulldist_corr


def _simplex_volume(simplex_vertices: np.ndarray) -> float:
...
def _trapezoid_rule_n_dim(
    ...
    for s in simplices:
        integral += _simplex_volume(x[s, :]) * np.sum(y[s])
...
def index_conversion(
    ...
    matching_indices = _matching_row_indices(search_corr, overenum_corr)
...
def orderparam_soft(
    ...
    return _trapezoid_rule_n_dim(
        predicted_x, predicted_y, translated_true_simplices
    ) - _trapezoid_rule_n_dim(predicted_x, predicted_y, predicted_simplices)


def orderparam_hard(
    imposed_gs_indices: np.ndarray,
    predicted_hull: ConvexHull,
):
    """For scenarios where samples outside of the 'true' cone is unacceptable. This function can be interpreted as
    a version of orderparam_soft() that is zero for ECIs within the 'true' cone, and infinite for ECIs that are outside
    of the true cone. This is done by explicitly checking ground state identities.

    Parameters
    ----------
    imposed_gs_indices:np.ndarray
        Indices of the imposed ground state structures. Indices MUST be paired to the same structures as they appear in the predicted_hull
        point set. If they do not match for any reason, the matched indices can be obtained using the index_conversion() function above, which
        will provide an index conversion dictionary.

    predicted_hull:ConvexHull
        The convex hull of the current predicted ground states, generated by the current ECI vector

    Returns
    -------
    0 or np.inf
        Returns 0 if the ECI is within the cone, returns np.inf if the ECI is outside of the cone.
    """
    missing = []
    spurious = []
    predicted_vertices, _ = lower_hull(predicted_hull)

    # Check for missing ground states in the predicted set
    for true_vertex in imposed_gs_indices:
        match = False
        for predicted_vertex in predicted_vertices:
            if true_vertex == predicted_vertex:
                match = True
                break
        if match == False:
            missing.append(true_vertex)

    # Check for spuruious ground states in the predicted set
    for predicted_vertex in predicted_vertices:
        match = False
        for true_vertex in imposed_gs_indices:
            if predicted_vertex == true_vertex:
                ...
                break
        if match == False:
            spurious.append(predicted_vertex)
...
def hull_simplices(hull: ConvexHull):
    """Extracts lower hull simplices from a convex hull object."""
    _, simplices = lower_hull(hull)
...
def orderparam_gradient(
    ...
    ----------
    imposed_gs_corr: np.ndarray
        Array of correlations for the set of configurations containing the imposed ground states. Shape (n,k) of n configurations, k eci.
        This set can contain the imposed ground states exactly (and no other configurations), or it can contain a super-set of configurations
        that contain the imposed ground states. Ground state identity is conveyed through imposed_gs_simplices.
    imposed_gs_comp: np.ndarray
        Array of compositions for each configuration in imposed_gs_corr. Shape (n,m) of n configurations, m composition dimensions.
    imposed_gs_simplices: np.ndarray
        Array of imposed simplices, shape (s,m+1) where s is the number of simplices, m is the number of composition dimensions
        (the +1 is for the energy dimension). Each element of this array is a configuration index corresponding to a lower convex hull vertex.
        This array can be obtained using the "hull_simplices()" function above. These will index into imposed_gs_corr, so any indices listed in
        imposed_gs_simplices MUST exist in imposed_gs_corr and imposed_gs_comp.
    overenum_corr: np.ndarray
    ...
    predicted_simplices: np.ndarray
        Array of predicted simplices. Always shape (q,m+1) where q is the number of simplices on the predicted lower hull, and m is the number
        of composition dimensions. The number of simplices q can vary depending on the number of predicted ground states determined by an
        ECI vector.

    Returns
    -------
    negative_gradient: np.ndarray
        Negative gradient of the order parameter. Points in the direction that minimizes the order parameter. Shape (k,) of k eci dimensions.
    """
    # Calculate individual contributions to the gradient
    imposed_grad = np.zeros(imposed_gs_corr.shape[1])
    predicted_grad = np.zeros(imposed_gs_corr.shape[1])
    for simplex in imposed_gs_simplices:
        imposed_grad += np.sum(imposed_gs_corr[simplex], axis=0) * _simplex_volume(
            imposed_gs_comp[simplex, :]
        )
    for simplex in predicted_simplices:
        predicted_grad += np.sum(overenum_corr[simplex], axis=0) * _simplex_volume(
