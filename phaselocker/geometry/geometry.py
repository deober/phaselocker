...
def orderparam_hard(
    ...
    ----------
    true_hull:ConvexHull
        The convex hull of the 'true' imposed ground states.

    predicted_hull:ConvexHull
        The convex hull of the ground states, generated by the current ECI vector

    index_conversion_dict:dict
        Dictionary relating point indices in the true_hull point set to indices in the predicted_hull set.
    Returns
    -------
    0 or np.inf
    ...
    missing = []
    spurious = []
    imposed_gs_indices, _ = lower_hull(true_hull)
    translated_true_indices = np.array(
        [index_conversion_dict[vertex] for vertex in imposed_gs_indices]
    )
    predicted_vertices, _ = lower_hull(predicted_hull)

    # Check for missing ground states in the predicted set
    for true_vertex in translated_true_indices:
        match = False
...
def orderparam_gradient(
    ...
    calc_simplices: ConvexHull = None,
    predicted_simplices: ConvexHull = None,
):
    ...
    calc_simplices: np.array
        Array of imposed simplices. Always shape (l,m) of l imposed ground state vertices, in m composition dimensions.
    predicted_simplices: ConvexHull
        Array of predicted simplices. Always shape (q,m) where q is the number of predicted ground state vertices, in m composition dimensions.
        The number of ground states q can vary depending on the number of predicted ground states determined by an ECI vector.
    ...
    for simplex in calc_simplices:
        calc_grad += np.sum(corr_calc[simplex], axis=0) * _simplex_volume(
            comp_calc[simplex, :]
        )
