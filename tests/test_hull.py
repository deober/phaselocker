import pytest
import numpy as np
from phaselocker.geometry import (
    _barycentric_coordinates,
    hull_distance_correlations,
    lower_hull,
    full_hull,
    _simplex_energy_equation_matrix,
    _lower_hull_simplex_containing,
    _simplex_volume,
    _trapezoid_rule_n_dim,
)
from tests.binary import (
    binary_points,
...
#Copied from https://github.com/Van-der-Ven-Group/thermocore
...
def test_simplex_energy_equation_matrix_binary(
    ...
    equation_matrix_result = _simplex_energy_equation_matrix(
        binary_hull, binary_lower_hull_simplex_indices
    )
...
def test_energy_equation_matrix_binary_vertical(binary_hull):
    ...
    with pytest.raises(ValueError):
        _simplex_energy_equation_matrix(binary_hull, range(len(binary_hull.simplices)))
...
def test_lower_hull_simplex_containing_binary(
    ...
    result = _lower_hull_simplex_containing(test_points, binary_hull)
    result_indices_provided = _lower_hull_simplex_containing(
...
def test_lower_hull_simplex_containing_binary_out_of_bounds(binary_hull):
    ...
    with pytest.raises(ValueError):
        _lower_hull_simplex_containing(test_points_low, binary_hull)
    test_points_high = np.array([9.5])
    with pytest.raises(ValueError):
        _lower_hull_simplex_containing(test_points_high, binary_hull)


def test_barycentric_coordinates_1D():
    """Tests _barycentric_coordinates for points in one dimension."""
    for a in np.linspace(-2, 2, 10):
        for d in np.linspace(1, 5, 10):
            b = a + d
            for p in np.linspace(a, b, 10):
                coordinates = _barycentric_coordinates(
...
def test_hull_correlation_calculator(
    ...
    reference_hull = full_hull(compositions=ZrN_FCC_composition_subset,energies=ZrN_FCC_formation_energy_subset)
...
def test_simplex_volume():
    ...
    assert np.isclose(_simplex_volume(synth_simplex), 1 / 2)
...
def test_trapezoid_rule_n_dim():
    ...
    assert np.isclose(
        _trapezoid_rule_n_dim(
            test_points[:, 0].reshape(-1, 1), test_points[:, 1], simplex_indices
        ),
        -2,
    )
